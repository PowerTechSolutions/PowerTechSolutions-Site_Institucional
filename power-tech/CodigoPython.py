...
try:
    ...
    if mydb.is_connected():
        ...
        if 0 == 1:
            sql_querryCPU = 'INSERT INTO Monitoramento_RAW VALUES (NULL, CURRENT_TIMESTAMP(), %s,0)'
            valCPU = [round(cpu,2)]
            mycursor.execute(sql_querryCPU, valCPU)
            mydb.commit()
        if 0 == 1:
            sql_querryRAM = 'INSERT INTO Monitoramento_RAW VALUES (NULL, CURRENT_TIMESTAMP(), %s,0)'
            valRAM = [round(ram.percent,2)]
            mycursor.execute(sql_querryRAM, valRAM)
            mydb.commit()
        if 1 == 1:
            sql_querryDISCO = 'INSERT INTO Monitoramento_RAW VALUES (NULL, CURRENT_TIMESTAMP(), %s,12)'
