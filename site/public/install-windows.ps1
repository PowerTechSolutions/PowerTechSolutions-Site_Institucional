  ...
  if (Get-Command $SLACK_CLI_NAME -ErrorAction SilentlyContinue) {
    ...
    if ($get_finger_print -ne $FINGERPRINT) {
      ...
      if (!($slack_cli_version -contains "Using ${SLACK_CLI_NAME}.exe v")) {
        ...
        Write-Host "Halting the install to avoid accidently overwriting it."
...
function install_slack_cli {
  ...
  try {
      ...
      try {
        ...
        if (!(Test-Path $alternative_slack_cli_dir)) {
          try {
            ...
            throw "`nInstaller cannot create folder in $($alternative_slack_cli_dir). `nPlease manaully create $($slack_cli_dir) folder and re-run the installation script"
          }
        }
      }
    }
    ...
    throw "`nInstaller cannot create folder for Slack CLI, `nPlease manaully create $($slack_cli_dir) folder and re-run the installation script"
  }
  try {
